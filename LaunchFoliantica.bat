...
if ($Help) {
    Write-Host ""
    Write-Host "  $(bold (cyan "Foliantica"))  -  Launcher"
    Write-Host ""
    ...
    Write-Host "    $(gray "- Opens two windows: backend (8765) + frontend (3000)")"
    Write-Host ""
...
Write-Host ""
Write-Host "  $(bold (cyan "Foliantica"))"
Write-Host "  $(gray "-------------------------------------")"
Write-Host ""
...
Start-Process powershell -ArgumentList @(
    ...
    "-Command", "& { `$host.UI.RawUI.WindowTitle = 'Foliantica [backend]'; Set-Location '$Root\api'; & '.\.venv\Scripts\python.exe' run.py --dev; Read-Host 'Stopped - press Enter to close' }"
)
...
Start-Process powershell -ArgumentList @(
    "-NoProfile", "-ExecutionPolicy", "Bypass",
    "-Command", "& { `$host.UI.RawUI.WindowTitle = 'Foliantica [frontend]'; Set-Location '$webDir'; `$env:LW_API_PORT = '8765'; npm run dev; Read-Host 'Stopped - press Enter to close' }"
)

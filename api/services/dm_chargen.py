...
def generate_name(name_style: str, rng: random.Random | None = None) -> str:
    rng = rng or random
    styles = load_ruleset()["name_styles"]
    style = styles.get(name_style) or styles["human"]
    first = rng.choice(style["first_start"]) + rng.choice(style["first_end"])
...
def build_character(
    ...
    """Assemble a character dict from six stat totals (highest→lowest by class priority).

    Returns {name, species, class, description bits, rpg_sheet} ready to store
    on a codex entry.
    """
    rules = load_ruleset()
    ...
    # Species bonus; "any" goes to the class's top priority
    ...
    sheet = {
        ...
        "ac": ac,
        "abilities": [sp["trait"], *cl["abilities"]],
        "conditions": [],
    ...
    return {
        "name": name or generate_name(sp["name_style"], rng),
        "species": sp["label"],
...
def generate_npc(species: str | None, klass: str | None, rng: random.Random | None = None) -> dict:
    """Random-fill any missing choice and build an NPC stat block (code-named, code-rolled)."""
    rng = rng or random
    rules = load_ruleset()
    ...
    return build_character(species, klass, totals, is_pc=False, rng=rng)
